...
# https://github.com/openai/guided-diffusion/blob/main/guided_diffusion/image_datasets.py
def random_crop_dual_arr(pil_image, pil_lq_image, image_size, min_crop_frac=0.8, max_crop_frac=1.0):
    ...
    while min(*pil_image.size) >= 2 * smaller_dim_size:
        ...
        pil_lq_image = pil_lq_image.resize(
            tuple(x // 2 for x in pil_lq_image.size), resample=Image.BOX
        )
    ...
    pil_lq_image = pil_lq_image.resize(
        tuple(round(x * scale) for x in pil_lq_image.size), resample=Image.BICUBIC
    )

    arr = np.array(pil_image)
    arr_lq = np.array(pil_lq_image)
