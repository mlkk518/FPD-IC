...
class ControlLDM(nn.Module):
    ...
    def load_pretrained_sd(
        ...
        modules = [("unet", self.unet), ("vae", self.vae)]
        used = set()
        missing = set()
    ...
    @torch.no_grad()
    def load_vae_from_ckpt(self, sd: Dict[str, torch.Tensor]) -> None:
    ...
    def vae_encode(
        ...
        if sample:
            if Stage2:
                z, enc_feats = encoder(image, Stage2)
                z = z.sample() * self.scale_factor
            else:
                z = encoder(image).sample() * self.scale_factor
        else:
            if Stage2:
                z, enc_feats = encoder(image, Stage2)
                z = z.mode() * self.scale_factor
            else:
    ...
    def vae_decode(
        ...
        return decoder(z / self.scale_factor, enc_feats, weight)
    ...
    def forward(self, x_noisy, t, cond):
        ...
        # c_txt = torch.zeros(x_lq.size(0), 77, 1024).to(x_lq.device) ## 低效率写法
        c_img = cond["c_img"]
        # print("c_img shape", c_img.shape)


        control = self.controlnet(x_lq=x_lq, x=x_noisy, hint=c_img, timesteps=t, context=c_txt)
