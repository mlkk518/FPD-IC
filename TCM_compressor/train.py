...
def main(argv):
    ...
    train_transforms = transforms.Compose(
        [ResizeIfSmallerThan(args.patch_size[0]),  transforms.RandomCrop(args.patch_size), transforms.RandomHorizontalFlip(p=0.5), transforms.RandomVerticalFlip(p=0.5),  transforms.ToTensor()]
    )

    test_transforms = transforms.Compose(
        [transforms.CenterCrop(args.patch_size), transforms.ToTensor()]
    )
    train_dataset = ImageFolder(args.dataset, split="Div2K_Flick2K_train_Size_512_640_800",  transform=train_transforms)  # FAIR1M_V2.0_P1_P2_img2_DOTA_V1.5_train
    test_dataset = ImageFolder(args.dataset, split="kodak_crop_256", transform=test_transforms)



    # train_dataset1 = ImageFolder(args.dataset, split="Train_DOTA_LoveDA_NWPU_256", transform=train_transforms)
    # test_dataset = ImageFolder(args.dataset, split="Test_20", transform=test_transforms)
    # train_dataset2 = ImageFolder(args.dataset, split="UCMerced_LandUse_split/train", transform=train_transforms)
    # train_dataset3 = ImageFolder(args.dataset, split="AID_split/train", transform=train_transforms)
    # train_dataset4 = ImageFolder(args.dataset, split="DOTA1.5/train", transform=train_transforms)

    # 合并数据集
    # combined_dataset = ConcatDataset([train_dataset2, train_dataset3, train_dataset4])
