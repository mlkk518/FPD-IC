...
def main():
    ...
    # 新增保存路径定义（根据实际需要修改路径）
    latent_path = save_path + "/latents"
    lq_path = save_path + "/LQ"
    ...
    for batch in loader:
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                ...
                z_sample = z.unsqueeze(0)  # 保持维度一致性 [1, C, H, W]
                z_sample = z_sample.data.cpu().numpy()  # 转换为numpy数组
                save_latent_path = os.path.join(latent_path, f"{img_name[0]}.npy")
                np.save(save_latent_path, z_sample)
                ...
                # 生成保存路径
                save_sp_path = os.path.join(sample_path, f"{img_name[0]}.png")
                save_gt_path = os.path.join(gt_path, f"{img_name[0]}.png")
                save_lq_path = os.path.join(lq_path, f"{img_name[0]}.png")

                # 保存图像
                save_image(sample, save_sp_path)
                save_image(lq_img, save_lq_path)
                save_image(gt_img, save_gt_path)

        if total_num > 10000: ## 1 W
            break

    print(f"Enjoy the results in {save_latent_path}! The Average PSNR is {np.mean(PSNR_List)} !")
