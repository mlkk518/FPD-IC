...
def main():
    ...
    if cfg.test.vae_ckpt:
        checkpoint = torch.load(cfg.test.vae_ckpt, map_location="cpu")
        if 'model_state_dict' in checkpoint:
            cldm.load_vae_from_ckpt(checkpoint['model_state_dict'])
        else:
            cldm.load_vae_from_ckpt(checkpoint)
        print(
            f"load VAE weight from checkpoint: {cfg.test.vae_ckpt}"
        )
    ...
    save_en_path = cfg.test.output + "En"
    os.makedirs(save_en_path, exist_ok=True)
    ...
    for batch in loader:
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                ...
                out_enhance = (pure_cldm.vae_decode(z, enc_feats=enc_feats) + 1) / 2

                lq_img = log_lq
                gt_img = (log_gt + 1)/ 2
                sample = torch.clamp(sample, 0, 1)
                out_enhance = torch.clamp(out_enhance, 0, 1)
                ...
                PSNR_EN = calculate_psnr((out_enhance * 255.0).cpu().numpy(), (gt_img * 255.0).cpu().numpy())
                PSNR_EN_List.append(PSNR_EN)
                # 生成保存路径
                ...
                en_img_path = os.path.join(save_en_path, f"{img_name[0]}.png")

                # 保存图像
                save_image(sample, sample_path)
                # save_image(lq_img, lq_img_path)
                save_image(out_enhance, en_img_path)
